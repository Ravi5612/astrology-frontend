...
export default function FreeEndPromptModal({
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
            <div className={`${isDarkMode ? 'bg-[#1a0c0c] border-white/10' : 'bg-neutral-900 border-white/10'} w-full max-w-sm rounded-[32px] overflow-hidden border p-8 text-center shadow-2xl relative text-white`}>
                {/* 30s Countdown Ring */}
                ...
                <div className="flex flex-col gap-3">
                    {freeLimitData?.requireRecharge ? (
                        ...
                        <button
                            onClick={() => {
                                socket?.emit('confirm_paid_continuation', { sessionId: parseInt(sessionId || '0') });
                            }}
                    ...
                    <button
                        ...
                        className={`w-full py-4 rounded-2xl border border-white/5 text-gray-400 font-bold text-xs uppercase tracking-widest hover:bg-red-500/5 hover:text-red-500 transition-all`}
                    >
                        End Call
                    </button>
                </div>
