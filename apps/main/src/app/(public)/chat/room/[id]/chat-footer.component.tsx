        ...
        <footer className={`flex-shrink-0 transition-all duration-300 ${isDarkMode ? 'bg-[#1a0c0c]/80 border-white/5' : 'bg-white/80 border-black/5'} backdrop-blur-xl border-t px-3 md:px-12 py-3 md:py-10 shadow-[0_-20px_50px_rgba(0,0,0,0.05)] overflow-visible`}>
            <div className={`relative flex items-center gap-2 md:gap-5 max-w-6xl mx-auto overflow-visible ${sessionStatus !== 'active' ? 'opacity-40 pointer-events-none grayscale' : ''}`}>
                ...
                <button
                    ...
                    className={`flex-shrink-0 w-9 h-9 md:w-16 md:h-16 rounded-[14px] md:rounded-[28px] flex items-center justify-center transition-all shadow-xl active:scale-90 group relative overflow-hidden ${inputValue.trim() || pendingAttachment ? "bg-[#fd6410] text-white hover:shadow-[#fd6410]/40 hover:-translate-y-1 hover:scale-105" : "bg-gray-100 text-gray-300 cursor-not-allowed"}`}
                >
                    ...
                    <Send className={`w-4 h-4 md:w-7 md:h-7 relative z-10 transition-transform ${inputValue.trim() || pendingAttachment ? "group-hover:-translate-y-1 group-hover:translate-x-1" : ""}`} />
                </button>
            </div>
