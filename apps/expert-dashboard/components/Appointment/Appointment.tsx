    ...
    try {
      ...
      const mappedAppointments: Appointment[] = await Promise.all(uniqueSessions.map(async (session: any) => {
        ...
        return {
          ...
          type: "new",
          reminder: false,
          meetingLink: isPuja ? `/dashboard/puja-session/${session.id}` : (isCall ? (session.type === 'video' ? `/dashboard/video/${session.id}` : `/dashboard/call/${session.id}`) : `/dashboard/chat/${session.id}`),
    ...
    if (isExpertAuthenticated && expertUser) {
      ...
      const handleNewRequest = (session: any) => {
        ...
        const newAppt: Appointment = {
          ...
          type: "new",
          reminder: false,
          meetingLink: `/dashboard/chat/${session.id}`,
      ...
      const handleNewCallRequest = (data: any) => {
        ...
        const newAppt: Appointment = {
          ...
          type: "new",
          reminder: false,
          meetingLink: session.type === 'video' ? `/dashboard/video/${session.id}` : `/dashboard/call/${session.id}`,
          sessionId: session.id,
          clientId: session.client_id || session.userId || session.clientId,
          expiresAt: session.expires_at || session.expiresAt,
          isFree: !!(session.is_free ?? session.isFree),
